...
void getdata_book()
{
    ...
    if(fp_book!=NULL)
    {
        while(!feof(fp_book))
        {
            ...
            fscanf(fp_book,"%s%d%d%d%s",temp_book->dept,&temp_book->book_id,&temp_book->status,&temp_book->time_issued,temp_book->book_name);
             fscanf(fp_book,"\n");
            if(start_book==NULL)
            {
...
void putdata_book()
{
    if(start_book!=NULL)
    {
        ...
        while(cur_book!=NULL)
        {
            fprintf(fp_book,"%s\t%d\t%d\t%d\t%s\n",cur_book->dept,cur_book->book_id,cur_book->status,cur_book->time_issued,cur_book->book_name);
            temp_book=cur_book;
            cur_book=cur_book->link_book;
            free(temp_book);
        }
...
void add_stu()
{
    ...
    while(cur_stu!=NULL)
    {
        ...
        {
            free(temp_stu);
            cout<<"provided id already exists so cannot add new student.";
    ...
    if(status==1)
    {
    cout<<"enter student name with id   "<<temp_stu->stu_id<<" : ";
    cin>>temp_stu->stu_name;
    temp_stu->fine=0;
...
void delete_stu()
{
    ...
    cout<<"enter unique id of student to delete student details from the system :  ";
    cin>>temp_id;
            ...
            while(status_fine)
            {
                if(cur_stu->fine!=0)
                {
                    ...
                    else
                    {
                        ...
                        cout<<" fine should be completely paid \n";
                        cur_stu->fine=0;
                        cout<<" fine updated to zero(0)";
                    }
                }
...
void add_book()
{
    ...
    temp_book=(struct book *)malloc(sizeof(struct book));
    temp_book->link_book=NULL;
    cout<<"enter new book id alloted to book to be add in library :  ";
    cin>>temp_book->book_id;
    while(cur_book!=NULL)
    {
...
void delete_book()
{
    ...
    temp_book=NULL;
    cur_book=start_book;
    cout<<"enter the unique book id of the book to be deleted :  ";
    cin>>temp_id;
    while(cur_book!=NULL)
    {
    ...
    else
    {
        ...
        else
        {
         ...
         cout<<"successfully deleted book from system.";
        ...
        else // book can be issued by the student.
        {
            ...
            while(status_fine)
            {
                ...
                else //  fine is zero.
                {
                    ...
                    temp_book=(struct book *)malloc(sizeof(struct book));
                    temp_book->link_book=NULL;
                    cout<<"\n enter id of the book to issue by student with student id = " << cur_stu->stu_id <<": ";
                    cin>>temp_book->book_id;
                    cur_book=start_book;
                    while(cur_book!=NULL)
                    {
...
void student_details()
{
    ...
    while(cur_stu!=NULL) // traverse the details of registered students
    {
        if(cur_stu->stu_id==temp_id)
        {
            ...
            cout<<"\n\n student name is "<<cur_stu->stu_name <<"\n fine is "<< cur_stu->fine<<"\n books issued is "<<cur_stu->books_issued <<"\n\n";
            break;
        }
        cur_stu=cur_stu->link_stu;
    }
    ...
    else // here cur_stu points to the student details. // means status is 1
    {
        ...
        if(cur_stu->books_issued!=0)
        {
            ...
            while(cur_issued!=NULL)
            {
                ...
                {
                    cout<<"book id="<<cur_issued->book_id <<" and book name="<< cur_issued->book_name<<"\n";
                }
                cur_issued=cur_issued->link_issued;
            }
        }
...
void dept_num()
{
    ...
    else
    {
        ...
        while(!feof(fp_dept))
        {
           fscanf(fp_dept,"%s%d%d",dept.dept_name,&dept.total,&dept.available);
           fscanf(fp_dept,"\n");
           cout<<"dept = "<<dept.dept_name <<" \t total installed books in  system = "<< dept.total<<" \t available in library = "<< dept.available<<" \n";
        }
    }
    fclose(fp_dept);
}
...
void find_book()
{
    ...
    cout<<"printing based on name will show number of books present with the same name case sensitive :\n";
    cin>>choice;
    switch(choice)
    {
    case 1:
    ...
    case 2:
        ...
        if(status==0)
            ...
            cout<<"\n number of books present in library with provided book name is :"<<count1 <<" \n";
        break;
    default:
...
bool compare(const pair<string,int> &f,const pair<string,int> &l)
{
...
void buy_new_books()
{
    ...
    FILE *fp_time;
    fp_time=fopen("book.txt","r");
    while(!feof(fp_time))// till file will not be closed
    {
        fscanf(fp_time,"%s%d%d%d%s",new_book.dept,&new_book.book_id,&new_book.status,&new_book.time_issued,new_book.book_name);
        int k=0;
        while(new_book.book_name[k]!='\0')
        {
            s=s+new_book.book_name[k];
            k++;
        }
        a=new_book.time_issued;
...
//        cout<<v[f].first<<" "<<v[f].second<<"\n";
    ...
    int budget;
    cin>>budget;
    cout<<"enter number of books of top priroity you want to buy\n";
    int num,sum;
    cin>>num;
    ...
    for(int f=0;f<num;f++)
    {
        cout<<"enter price of book with name "<< v[f].first<<endl;
        int price;
        cin>>price;
        if(price<minprice)
           {
        ...
        nv.push_back(make_pair(v[f].first,price));
    }
    int *noofbooks=new int[num]();
    int carry;
    ...
    for(int f=0;f<num;f++)
    {
        cout<<nv[f].first<<" "<<noofbooks[f]<<endl;
    }
    if(budget!=0)
        cout<<"budget left is "<<budget;

}

int main()
{
    char pass[55]="college";
    char password[55];
    cout<<"password : \n";
    int run=0;
    do
